...
export default function Header({ currentPage, onNavigate, isAdmin }: Props) {
  ...
  const navLinks: { label: string; page: Page }[] = [
    ...
    { label: 'Write for Us', page: 'submit' },
  ];
    ...
    >
          ...
          {navLinks.map(({ label, page }) => (
            <button
              ...
              className={`text-sm font-medium transition-colors hover:text-pink-500 ${
                currentPage === page ? 'text-pink-500' : 'text-gray-600'
              }`}
            >
              {label}
            </button>
          ...
          <div className="relative" ref={subscribeRef}>
            ...
            {subscribeOpen && (
              <div className="absolute right-0 top-full mt-3 w-72 bg-white rounded-2xl shadow-xl border border-pink-100 p-5 z-50">
                <p className="text-sm font-bold text-gray-800 mb-1">Get articles in your inbox</p>
                <p className="text-xs text-gray-500 mb-4">Join our free weekly newsletter</p>
                <SubscribeInline />
              </div>
            )}
          </div>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-pink-50 px-6 py-5 flex flex-col gap-5">
          ...
          <div>
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">Newsletter</p>
            <SubscribeInline />
          </div>
          {isAdmin && (
            <button
